...
async function insertMedicines() {
  console.log('🚀 Inserting medicines via backend API...');
  ...
  for (const medicine of medicines) {
    try {
      ...
      if (response.ok) {
        console.log(`✅ ${medicine.product_id} - ${medicine.name}`);
        successCount++;
      } else {
        console.log(`❌ ${medicine.product_id} - Error: ${data.message || 'Unknown error'}`);
        failCount++;
      }
    } catch (error) {
      console.log(`❌ ${medicine.product_id} - Error: ${error.message}`);
      failCount++;
    }
  }
  ...
  console.log(`\n✨ Medicines are now available for verification!`);
}
